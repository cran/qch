...
integral.kde_adapted <- function(q, fhat, density, tol = 1e-10)
{
  ...
  last_incomplete_trapezoid_arma(q.prob, q.ind, q, fhat$eval, fhat$est, simp.rule, density)
  
  if (density) q.prob[q.prob>=1] <- 1;
  
  ##------- C++ 1st version (slower) ---------
  # Handling the index ordering of q in C++ makes it slower ...
  # remove_decreasing_values_cpp_slow_ordering(q.prob, q, tol)
  ##------------------------------------------

  ##------- C++ 2nd version (faster) ---------
  # Index ordering of q is done first because the R base function order()
  # is twice as fast as the C++ function arma::sort_index()
  # CAUTIOUS: order(q)-1 is necessary because of C++ indexing 
  order.q <- order(q)-1
  remove_decreasing_values_cpp(q.prob, order.q, tol)
  ##------------------------------------------
