...
#' @param memory_efficient_EM Logical. If `TRUE`, use the memory-efficient implementation of the EM algorithm.
#' This option is recommended for large datasets, as it reduces memory usage at the cost of additional computation time.
#' The memory-efficient implementation is automatically used when the size of the \code{posterior} matrix exceeds 2 GB.
#' @param threads_nb Integer. Number of threads to use for parallel computation.
#' This parameter is only used when \code{memory_efficient_EM = TRUE}.
#' By default, the number of threads is set to the number of available CPU cores.
#' @param plotting Logical. Should some diagnostic graphs be plotted ? Default is \code{FALSE}.
...
#'
#' \itemize{
#' \item If the memory efficient version of EM algorithm is not used, the list will additionally contain:
#' \tabular{ll}{
...
qch.fit <- function(pValMat, EffectMat = NULL, Hconfig, copula = "indep", memory_efficient_EM =FALSE, threads_nb = 0, plotting = FALSE, Precision = 1e-6) {
  n <- nrow(pValMat)
  Q <- ncol(pValMat)
  ...
  ## Memory management
  if (memory_efficient_EM == TRUE | n * (2**Q) * 8 > 2e9) { # if the storage requirement exceeds 2Go
  ...
  p0 <- rep(0, Q)
  for (q in 1:Q) {
    p0[q] <- min(qvalue_pi0est(p = pValMat[, q], pi0.method = "bootstrap")$pi0, 1 - 1 / n)
  }
  SomeH1 <- which(p0 < 1)
  NoH1 <- which(p0 == 1 - 1 / n)
  if (length(NoH1) == 1) {
